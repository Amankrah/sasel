...
const FEATURED_QUERY = `*[_type == "publication"] | order(year desc, month desc) [0...5] {
  _id,
  title,
  slug,
  publicationType,
...
function formatAuthors(pub: SanityPublication): string {
  ...
  return labAuthors || ext || 'Unknown Authors';
}

function formatVenue(pub: SanityPublication): string {
  if (pub.journal) {
    let v = pub.journal;
    if (pub.volume) v += ` ${pub.volume}`;
    if (pub.issue) v += ` (${pub.issue})`;
...
export default function PublicationsShowcase() {
  ...
  useEffect(() => {
    let cancelled = false;
    client
      .fetch<SanityPublication[]>(FEATURED_QUERY)
      .then((data) => {
        if (!cancelled) setPublications(data || []);
      })
      .catch((err) => console.error('PublicationsShowcase fetch failed:', err))
      .finally(() => {
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="max-w-6xl mx-auto mb-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="backdrop-blur-md bg-white/50 border border-white/60 rounded-2xl shadow-xl p-6 text-center hover:shadow-2xl transition-all hover:scale-105">
              ...
              <div className="text-gray-600 font-semibold uppercase tracking-wider text-sm">Total Citations</div>
            ...
            <div className="backdrop-blur-md bg-white/50 border border-white/60 rounded-2xl shadow-xl p-6 text-center hover:shadow-2xl transition-all hover:scale-105">
              ...
              <div className="text-gray-600 font-semibold uppercase tracking-wider text-sm">Citations</div>
              <div className="text-xs text-gray-500 mt-2">📈 Since 2020</div>
            </div>
            <div className="backdrop-blur-md bg-white/50 border border-white/60 rounded-2xl shadow-xl p-6 text-center hover:shadow-2xl transition-all hover:scale-105">
              <div className="text-5xl font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent mb-2">
                {scholarMetrics.hIndex}
              </div>
              <div className="text-gray-600 font-semibold uppercase tracking-wider text-sm">h-index</div>
              <div className="text-xs text-gray-500 mt-2">🎯 Impact metric</div>
            </div>
            <div className="backdrop-blur-md bg-white/50 border border-white/60 rounded-2xl shadow-xl p-6 text-center hover:shadow-2xl transition-all hover:scale-105">
              ...
              <div className="text-gray-600 font-semibold uppercase tracking-wider text-sm">i10-index</div>
              <div className="text-xs text-gray-500 mt-2">⭐ Publications with 10+ citations</div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto mb-12">
          <div className="backdrop-blur-md bg-white/50 border border-white/60 rounded-2xl shadow-xl p-8">
            <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">Research Areas</h3>
        ...
        <div className="max-w-6xl mx-auto mb-12">
          ...
          {loading ? (
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                    <div className="flex-1">
                      <h4 className="text-lg font-semibold text-gray-900 mb-2 leading-tight">{pub.title}</h4>
                      <p className="text-sm text-gray-600 mb-2">{formatAuthors(pub)}</p>
                      <div className="flex flex-wrap items-center gap-3 text-sm">
                        {pub.journal && <span className="text-gray-700 font-medium">📖 {formatVenue(pub)}</span>}
                        {pub.conference && !pub.journal && (
                          <span className="text-gray-700 font-medium">🎤 {pub.conference}</span>
                        )}
                        ...
                        {pub.publicationType && (
                          ...
                          >
                            {pub.publicationType === 'JOURNAL'
                              ...
                              : pub.publicationType === 'BOOK'
                              ? '📚 Book'
                              : pub.publicationType === 'CHAPTER'
                              ? '📖 Chapter'
                              : pub.publicationType === 'THESIS'
                              ? '🎓 Thesis'
                              : pub.publicationType}
                          </span>
                        )}
            ...
            <div className="backdrop-blur-md bg-white/50 border border-white/60 rounded-xl shadow-lg p-12 text-center">
              ...
              <p className="text-gray-600">Publications will appear here once they are added.</p>
            </div>
          )}
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              <div className="flex flex-col items-center">
                ...
                <p className="text-gray-600 text-sm">View complete publication list and citation metrics</p>
              </div>
            </a>
            <Link
              href="/publications"
            ...
            >
              <div className="flex flex-col items-center">
                ...
                <p className="text-gray-600 text-sm">Browse our complete publication archive</p>
              </div>
            </Link>
          </div>
        </div>
