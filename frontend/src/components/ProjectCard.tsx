...
export default function ProjectCard({ project, variant = 'default' }: ProjectCardProps) {
  if (variant === 'compact') {
    return (
      <div className={`backdrop-blur-md bg-white/60 border border-white/80 rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] ${!project.is_active ? 'opacity-90' : ''}`}>
        {project.image ? (
        ...
        <div className="p-6">
          ...
          <p className="text-gray-700 mb-4 line-clamp-3 text-sm leading-relaxed">{project.description}</p>
          ...
          <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-gray-700 font-medium">
                  {project.start_date && project.end_date ? (
                    ...
                    `Ended ${new Date(project.end_date).getFullYear()}`
                  ) : ''}
                </span>
              </div>
            )}
            ...
            {/* Action Buttons */}
            {(project.website || project.github_repo) && (
  ...
  return (
    <div className={`backdrop-blur-md bg-white/60 border border-white/80 rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 hover:scale-[1.01] ${!project.is_active ? 'opacity-90' : ''}`}>
      {project.image ? (
          ...
          <div className="flex gap-3 pt-2 border-t border-gray-200">
            {project.website && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 inline-flex items-center justify-center px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-medium rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all shadow-md hover:shadow-lg"
              >
                Visit Website
